...
if(isset($_POST['sbm'])){
    ...
    // hinh anh;
    $image_us = $_FILES['image_us']['name'];
    $image_us_tmp = $_FILES['image_us']['tmp_name'];
    $image_us = time().'_'.$image_us;
    move_uploaded_file($image_us_tmp,'anh/'.$image_us);
    //pass
    ...
    //  while ($row = mysqli_fetch_array($query)) {
        if (mysqli_num_rows(mysqli_query($conn, "SELECT*FROM user WHERE user_mail = '$user_mail'")) == 0) {
            if ($user_pass == $user_re_pass) {
                $user_mail = $_POST["user_mail"];
                echo $sql_user = "INSERT INTO user( user_full, user_mail, user_pass,image_us, user_level) value( '$user_full', '$user_mail', '$user_pass','$image_us' ,'$user_level')";
                    ...
                    <div class="col-md-8">
                        ...
                        <form role="form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input name="image_us" required type="file" class="form-control">
                            </div>
                            <div class="form-group">
                                <label>Quyền</label>
